...
function setTelefoni(data) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		if (jwt_token) {
			row.appendChild(createTableCell(data[i].os));
			...
			buttonDelete.addEventListener("click", deleteTelefoni);
	...
	div.appendChild(table);
     ...
     // Dodajte stil za prvi red tabele
     table.querySelector('tr:first-child').style.backgroundColor = '#90EE90'; /* Zelena boja */
...
// 	var url = host + port + telefoniEndpoint + editId.toString();
// 	var headers = { };
// 	if (jwt_token) {
// 		headers.Authorization = 'Bearer ' + jwt_token;
// 	}
...
// 					document.getElementById("sellerLastName").value = data.lastName;
// 					document.getElementById("sellerYear").value = data.year;
// 					document.getElementById("sellerStore").value = data.storeId;
// 					editingId = data.id;
// 					formAction = "Update";
// 				});
// 			} else {
